...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      // Normalizing PHI between [-pi, pi]
      phi = atan2 ( sin ( phi ), cos ( phi ) ) ;

      // Convert each coordinate from polar to cartesian
      float x  	  = rho * cos(phi);
      float y  	  = rho * sin(phi);
      float vel_x = rho_dot * cos(phi);
      float vel_y = rho_dot * sin(phi);

      ekf_.x_ << x, y, vel_x, vel_y;
  ...
  // State transition matrix update adding the dt (delta time) from previous to now prediction
